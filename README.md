...
**Pre-release.** v0.x is **Windows-only**; Linux and Android are next.
...
relies on a locally-built `libyse.dll`. Until the native-assets build
hook lands you need a manual setup:
...
# Build libyse.dll once. Requires MSYS2 Clang64 (the same toolchain YSE
# itself uses) and CMake.
